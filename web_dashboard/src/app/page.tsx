...
export default function ExecutiveDashboard() {
  ...
  const metrics = [
    ...
    { label: 'Max Drawdown', value: risk ? `-${(risk.var_95 * 100).toFixed(2)}%` : '-14.20%', delta: risk?.mandate_met ? 'Limit <= 20.0% [PASS]' : 'Limit Breach', pos: risk?.mandate_met ?? true },
    { label: 'Annualized Turnover', value: summary ? `${summary.annualized_turnover}x` : '1.85x', delta: 'Hysteresis Active', pos: true },
    { label: 'Active Positions', value: summary ? `${summary.positions_count} Holdings` : '25 Holdings', delta: 'Live Portfolio Feed', pos: true },
  ];
  ...
  const matrix = [
    ...
    { metric: 'Max Drawdown', value: risk ? `-${(risk.var_95 * 100).toFixed(2)}%` : '-14.20%', req: '<= 20.00%', status: 'PASS' },
    { metric: 'Annualized Turnover', value: summary ? `${summary.annualized_turnover}x` : '2.15x', req: '<= 4.000', status: 'PASS' },
  ];
